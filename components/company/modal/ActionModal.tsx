...
const ActionModal = ({ title, description, bname, setOpen }) => {
  const router = useRouter();

  const deployToVercel = () => {
    fetch(
      `https://api.vercel.com/v1/integrations/deploy/prj_3jh0khhhsdQr7KSslDXr9k9hYTqV/miBC5ZOdiR`
    )
      .then((res) => res.json())
  ...
  return (
    <div className='max-w-screen fixed left-0 bottom-0 flex h-full w-full items-center justify-center bg-gray-800 xl:pl-24'>
      <div className='w-full max-w-xs rounded-lg bg-white sm:max-w-md'>
        <div className='flex flex-col items-start p-4'>
          ...
          <FlexRow className='ml-auto space-x-2'>
            {/* <form action="/api/deploy" method="POST" className="h-full"> */}
            <button
